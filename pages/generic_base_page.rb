# The base page for all SportNgin pages (excluding Tournament)
class GenericBasePage
  ...
  def self.page_url(url)
    define_method 'at?' do
      at = url.kind_of?(Regexp) ? url =~ @browser.url : url == @browser.url
      raise "Not at expected url: #{url}" unless at
    end
  end
